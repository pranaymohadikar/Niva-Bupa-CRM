"""
Niva Bupa CRM Server (SQLite + Staff IDs) — Changed 2026-05-04
================================================================
When updating coach/HP/CS fields, resolves staff name to ID and stores both.
"""

import hashlib
import shutil  # For database backup — Added 2026-05-08
import os
...
# ─── DATABASE BACKUP — Changed 2026-05-09 ───
# On Vercel: triggered via cron job hitting /api/cron/backup every 12 hours.
# Locally: can still be triggered manually via POST /api/backup.
# Uses pg_dump for PostgreSQL (Supabase) or file copy for SQLite.

BACKUP_DIR = BASE_DIR / "backups"
BACKUP_KEEP = 14                 # Keep last 14 copies (7 days of 12h backups)
DB_FILE = BASE_DIR / "crm.db"

IST_TZ = timezone(timedelta(hours=5, minutes=30))
CRON_SECRET = os.getenv("CRON_SECRET", "")  # Optional: secure cron endpoints

def run_backup():
    """Run a database backup. For SQLite: file copy. For Postgres: Supabase handles backups."""
    if DATABASE_URL.startswith("postgresql"):
        print("  [Backup] Supabase PostgreSQL — backups managed by Supabase dashboard")
        return {"status": "ok", "message": "Supabase manages backups automatically"}
    # SQLite file backup (local dev only)
    try:
        if DB_FILE.exists():
            ...
            print(f"  [Backup] ✓ crm_{ts}.db ({size_kb} KB)")
            backups = sorted(BACKUP_DIR.glob("crm_*.db"), key=lambda f: f.name, reverse=True)
            for old in backups[BACKUP_KEEP:]:
                old.unlink()
            return {"status": "ok", "file": dest.name, "size_kb": size_kb}
    except Exception as e:
        print(f"  [Backup] ERROR: {e}")
        return {"status": "error", "message": str(e)}

# Vercel Cron endpoint for backup
...
async def on_startup():
    print("  [Server] Started — backup cron configured on Vercel")
...
# ─── ADMIN ENDPOINT: Manual backup — Changed 2026-05-09 ───

@app.post("/api/backup")
async def trigger_backup():
    """Admin: trigger an immediate backup."""
    result = run_backup()
    return result
...
def patient_to_dict(p: Patient, db: Session) -> dict:
    ...
    result = {
        ...
        # Comments — Added 2026-05-06
        "comments": [{"id":c.id,"user":c.user_name,"role":c.user_role,"text":c.text,
                       "created_at":str(c.created_at)} for c in db.query(Comment).filter(
...
async def serve_frontend():
    html_path = BASE_DIR / "crm.html"
    if not html_path.exists(): return JSONResponse({"error": "crm.html not found"}, status_code=404)
...
async def get_patients(db: Session = Depends(get_db)):
    return [patient_to_dict(p, db) for p in db.query(Patient).all()]
...
if __name__ == "__main__":
    ...
    db_label = "Supabase (PostgreSQL)" if is_pg else f"crm.db ({Path('crm.db').stat().st_size//1024} KB)" if Path("crm.db").exists() else "NOT FOUND"
    backup_count = len(list(BACKUP_DIR.glob("crm_*.db"))) if BACKUP_DIR.exists() else 0
    print("\n" + "="*50)
    print("  Niva Bupa CRM Server")
    print("="*50)
    print(f"  Database: {db_label}")
    if not is_pg:
        print(f"  Backups:  {backup_count} files in backups/ (every 12h, keep {BACKUP_KEEP})")
    else:
        print(f"  Backups:  Managed by Supabase")
